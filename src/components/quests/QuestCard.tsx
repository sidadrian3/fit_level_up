...
export function QuestCard({ quest, className = "", onClaim }: QuestCardProps) {
    ...
    return (
        ...
        >
            {/* Status bar at top */}
            <div className={`h-1 w-full ${isClaimed ? "bg-border/30" : isCompleted ? "bg-accent-green" : "bg-border/40"}`} />

            <div className="p-5 flex flex-col gap-4 flex-1">
                {/* Header row: icon + title + XP badge */}
                <div className="flex items-start justify-between gap-3">
                    <div className="flex items-start gap-3">
                        <div className={`p-2 rounded-lg border ${isCompleted && !isClaimed ? "bg-accent-green/10 border-accent-green/30 text-accent-green" : "bg-background border-border text-muted"}`}>
                            <Icon size={20} />
                        </div>
                        <div>
                            <h4 className={`font-display font-bold text-lg uppercase tracking-tight leading-tight ${isClaimed ? "line-through text-muted" : "text-foreground"}`}>
                                {quest.title}
                            </h4>
                    ...
                    <div className="shrink-0 inline-flex items-center gap-1 bg-accent-green/10 border border-accent-green/20 px-2.5 py-1">
                        ...
                        <span className="font-display font-bold text-sm text-accent-green leading-none">+{quest.xpReward}</span>
                    </div>
                </div>
                    ...
                    {isCompleted ? (
                        isClaimed ? (
                            <div className="flex items-center gap-2 text-accent-green">
                                <CheckCircle size={14} />
                                <span className="text-xs font-bold uppercase tracking-widest">Claimed</span>
                            </div>
                        ) : (
                            <div className="flex flex-col gap-2">
                                ...
                                >
                                    {isClaiming ? "Claiming..." : error ? "Error — Retry" : `Claim ${quest.xpReward} XP`}
                                </button>
                        ...
                        <div className="space-y-2">
                            <div className="flex justify-between items-end">
                                ...
                                <span className="font-display font-bold text-xl tracking-tight text-foreground leading-none">
                                    {quest.progress} <span className="text-muted text-base">/ {quest.target}</span>
                                </span>
                            </div>
                            <ProgressBar
