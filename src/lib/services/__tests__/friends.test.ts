...
describe('friends Service Layer Test', () => {
    ...
    describe('sendFriendRequest', () => {
        ...
        it('Re-request after decline creates a new document successfully', async () => {
            ...
            await declineFriendRequest(req.id, userB.id); 
    ...
    describe('acceptFriendRequest', () => {
        it('Status becomes "accepted" and parties appear in getFriends', async () => {
            const req = await sendFriendRequest(userA.id, userB.id);
            const accepted = await acceptFriendRequest(req.id, userB.id);
        ...
        it('Throws when requester tries to accept their own request', async () => {
            const req = await sendFriendRequest(userA.id, userB.id);
            await expect(acceptFriendRequest(req.id, userA.id)).rejects.toThrow("Not authorized");
        });

        it('Throws conflict when already accepted', async () => {
            const req = await sendFriendRequest(userA.id, userB.id);
            await acceptFriendRequest(req.id, userB.id);
            await expect(acceptFriendRequest(req.id, userB.id)).rejects.toThrow("Friend request is not pending");
        });
    ...
    describe('declineFriendRequest', () => {
        it('Status becomes "declined" and not in getFriends', async () => {
            const req = await sendFriendRequest(userA.id, userB.id);
            const declined = await declineFriendRequest(req.id, userB.id);
        ...
        it('Throws when requester tries to decline their own request', async () => {
            const req = await sendFriendRequest(userA.id, userB.id);
            await expect(declineFriendRequest(req.id, userA.id)).rejects.toThrow("Not authorized");
        });
    });
    ...
    describe('removeFriend', () => {
        it('Requester or Receiver can remove -> { success: true }', async () => {
            ...
            await acceptFriendRequest(req1.id, userB.id);
            
            const res = await removeFriend(req1.id, userA.id);
            expect(res.success).toBe(true);

            const req2 = await sendFriendRequest(userA.id, userC.id);
            await acceptFriendRequest(req2.id, userC.id);

            const res2 = await removeFriend(req2.id, userC.id);
        ...
        it('Throws Not authorized when a third user tries to remove', async () => {
            const req1 = await sendFriendRequest(userA.id, userB.id);
            await expect(removeFriend(req1.id, userC.id)).rejects.toThrow("Not authorized");
        });
    });
